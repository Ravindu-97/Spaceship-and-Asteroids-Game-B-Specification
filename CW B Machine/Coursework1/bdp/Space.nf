...
THEORY ListConstantsX IS
  List_Valuable_Constants(Machine(Space))==(space_x_axis,space_y_axis,space,freeSpace,asteroids,homebase,starbase);
  Inherited_List_Constants(Machine(Space))==(?);
  List_Constants(Machine(Space))==(space_x_axis,space_y_axis,space,freeSpace,asteroids,homebase,starbase)
END
&
THEORY ListSetsX IS
...
THEORY ListPropertiesX IS
  ...
  List_Properties(Machine(Space))==(space_x_axis = 1..12 & space_y_axis = 1..7 & space = space_x_axis*space_y_axis & asteroids <: space & asteroids = {3|->2,3|->5,5|->4,6|->7,7|->1,7|->5,7|->7,8|->3,10|->6,11|->2,12|->5} & freeSpace <: space & freeSpace/\asteroids = {} & freeSpace\/asteroids = space & starbase: freeSpace & starbase = 6|->4 & homebase: freeSpace & homebase = 1|->1)
END
&
...
THEORY ConstantsEnvX IS
  Constants(Machine(Space)) == (Type(space_x_axis) == Cst(SetOf(btype(INTEGER,"[space_x_axis","]space_x_axis")));Type(space_y_axis) == Cst(SetOf(btype(INTEGER,"[space_y_axis","]space_y_axis")));Type(space) == Cst(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(freeSpace) == Cst(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(asteroids) == Cst(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(homebase) == Cst(btype(INTEGER,?,?)*btype(INTEGER,?,?));Type(starbase) == Cst(btype(INTEGER,?,?)*btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
